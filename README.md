...
- One release JAR (`WorldAreaReset-1.14.7.jar`) with English defaults, a Chinese configuration template, and built-in language support.
...
WorldAreaReset 1.14.7 is tested and supported only on Paper/Folia 1.21.11. Other Minecraft versions, Spigot, CraftBukkit, Purpur, and hybrid/modded servers are untested and receive no compatibility guarantee. `api-version: 1.21` is Bukkit metadata and does not expand this tested-version range.

## Install

1. Download `WorldAreaReset-1.14.7.jar` from the [latest release](https://github.com/Lazyzouo/WorldAreaReset/releases/latest). It starts with English defaults; set `language: "zh_CN"` to apply the Simplified Chinese configuration template.
...
The official templates keep one flat `messages` section containing only the selected language. Server-specific cleanup parameters retain safe official presets. Legacy nested language sections, `formatting`, `inline-replacements`, and root `gradient-colors` are removed during format 23 migration. Ordinary messages use the fixed KitLoader gradient, with the documented banner, severity, and help-heading color exceptions; console updater notices and cleanup/restoration broadcasts use only their single severity color and never a gradient. Player broadcasts retain the configured message styling.
...
recreate:
  ...
  countdown_seconds: 10
  player_protection_radius: 2 # spherical blocks protected around each player
  blocks_per_tick: 4096 # maximum blocks per Region tick; player protection remains local to nearby blocks
  worlds:
    - name: "arena_world"
...
`WorldAreaReset-1.14.7.jar` is written to `build/libs/`. The build verifies that its packaged English `config.yml` and Chinese `config-zh_CN.yml` match the official source configurations.
...
- 单个通用发布包 (`WorldAreaReset-1.14.7.jar`) 内置英文默认配置和中文配置模板，运行时只保留当前语言。
...
WorldAreaReset 1.14.7 仅在 Paper/Folia 1.21.11 上测试并提供兼容支持，需要 Java 21 或更高版本。其他 Minecraft 版本、Spigot、CraftBukkit、Purpur 及混合/模组服务端均未测试，不提供兼容保证。`plugin.yml` 中的 `api-version: 1.21` 只是 Bukkit 元数据，不代表所有 1.21.x 版本均受支持。

## 安装

1. 从[最新 Release](https://github.com/Lazyzouo/WorldAreaReset/releases/latest)下载 `WorldAreaReset-1.14.7.jar`。默认使用英文；将 `language` 设为 `zh_CN` 后重载即可应用简体中文配置模板。
2. 放入服务器 `plugins` 目录。
3. 启动一次服务器。
...
所有默认消息统一使用 KitLoader 五色渐变 `#FFB7D5`、`#D7C7FF`、`#B9E7FF`、`#D7C7FF`、`#FFB7D5` 和 `✧` 分割线。消息支持 `<#RRGGBB>`、`<gradient:...>`、`<bold>`、`<italic>`、`<underlined>`、`<strikethrough>`、`<obfuscated>`、`<reset>` 和 `<br>`，旧版 `&` 色码仍可使用。玩家端清理/恢复广播保留配置样式；后台端会移除渐变和颜色标签，按 severity 使用固定单色。
