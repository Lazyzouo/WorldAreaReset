...
> Applies to / 适用于：`WorldAreaReset 1.14.5`<br>
...
| `language` | `en_US` | The single `WorldAreaReset-1.14.5.jar` starts with English defaults; set `zh_CN` to apply the Simplified Chinese configuration template |
...
| `language` | `en_US` | 单个 `WorldAreaReset-1.14.5.jar` 默认使用英文；设为 `zh_CN` 后重载即可应用简体中文配置模板 |
